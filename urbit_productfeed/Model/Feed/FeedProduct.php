...
class Urbit_Productfeed_FeedProduct
{
    ...
    {
        return Urbit_Productfeed_Fields_Factory::processAttribute($this, $name);
    }
    ...
    {
        return Urbit_Productfeed_Fields_Factory::processAttributeByKey($this, $key);
    }
    ...
    protected function processId()
    {
        ...
        if ($id = $this->_processAttribute('URBIT_PRODUCTFEED_ATTRIBUTE_ID')) {
    ...
    {
        if ($name = $this->_processAttribute('URBIT_PRODUCTFEED_ATTRIBUTE_NAME')) {
            $this->name = $name;
        }
    }
    ...
    {
        if ($description = $this->_processAttribute('URBIT_PRODUCTFEED_ATTRIBUTE_DESCRIPTION')) {
    ...
    {
        if ($gtin = $this->_processAttribute('URBIT_PRODUCTFEED_ATTRIBUTE_GTIN')) {
            $this->gtin = $gtin;
        }
    }
    ...
    {
        if ($mpn = $this->_processAttribute('URBIT_PRODUCTFEED_ATTRIBUTE_MPN')) {
            $this->mpn = $mpn;
        }
    }
    ...
    protected function processDimensions()
    {
        ...
        if (floatval($heightValue = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_HEIGHT_VALUE'))) {
            $heightUnit = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_HEIGHT_UNIT')  ?: static::DEFAULT_UNIT;
        ...
        if (floatval($lengthValue = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_LENGTH_VALUE'))) {
            $lengthUnit = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_LENGTH_UNIT')
                ?: static::DEFAULT_UNIT;
        ...
        if (floatval($widthValue = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_WIDTH_VALUE'))) {
            $widthUnit = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_WIDTH_UNIT') ?: static::DEFAULT_UNIT;
        ...
        if (floatval($weightValue = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_WEIGHT_VALUE'))) {
            $weightUnit = $this->_processAttribute('URBIT_PRODUCTFEED_DIMENSION_WEIGHT_UNIT') ?: static::DEFAULT_WEIGHT_UNIT;
    ...
    protected function getPrice($productId, $taxRate, $combId = null, $useReduction = true)
    {
        ...
        $price = number_format(
            Product::getPriceStatic(
                $productId, $useTax, ($combId ? $combId : null),
                6, null, false, $useReduction
            ),
            2, '.', ''
        );
    ...
    protected function processPrices()
    {
        ...
        $regularPrice = $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_REGULAR_PRICE_VALUE', 'calc_RegularPrice');
        $salePrice    = $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_SALE_PRICE_VALUE', 'calc_SalePrice');

        //regular price
        $prices[] = [
            'currency' => $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_REGULAR_PRICE_CURRENCY', 'calc_Currency'),
            'value'    => $regularPrice,
            'vat'      => $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_REGULAR_PRICE_VAT', 'calc_TaxRate'),
            'type'     => 'regular',
        ];
        ...
        if ($salePrice != $regularPrice) {
            $sPrice = [
                'currency' => $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_SALE_PRICE_CURRENCY', 'calc_Currency'),
                'value'    => $salePrice,
                'vat'      => $this->_processAttributeOrByKey('URBIT_PRODUCTFEED_SALE_PRICE_VAT', 'calc_TaxRate'),
                'type'     => 'sale',
            ];

            if ($salePriceDate = $this->_processAttribute('URBIT_PRODUCTFEED_PRICE_EFFECTIVE_DATE')) {
        ...
        if ($additionalAttributes) {
            foreach ($additionalAttributes as $attribute) {
                if (isset($attribute['name'])) {
                    $cls = Urbit_Productfeed_Fields_Factory::getFieldClassByFieldName($attribute['name']);

                    switch ($cls) {
                        case 'Urbit_Productfeed_Fields_FieldDB' :
                            $value = Urbit_Productfeed_Fields_FieldDB::processAttribute($this, $attribute['name']);
                        ...
                        case 'Urbit_Productfeed_Fields_FieldAttribute':
                        ...
                        case 'Urbit_Productfeed_Fields_FieldFeature':
    ...
    protected function getConfigureValueByName($name)
    {
        ...
        $key .= $prefix != 'dimension' && $prefix != 'weight' ? 'ATTRIBUTE_' . strtoupper($name) : strtoupper($name);
